...
def pngs_to_mp4(folder, bg, size='800x800'):
    ...
    mp4name = '_'.join(outname.split('_')[:-1]) + '.mp4'
    call(['ffmpeg', '-y', '-f', 'image2', '-pattern_type', 'glob', '-i', folder + '*.jpg',  '-vcodec', 'libx264', '-pix_fmt', 'yuv420p',  '-vf', 'scale=trunc(iw/2)*2:trunc(ih/2)*2', mp4name])

    # webmname = mp4name.replace('.mp4', '.webm')
    # call(['ffmpeg', '-y', '-i', mp4name, '-c:v', 'libvpx-vp9', '-lossless', '1', webmname])
